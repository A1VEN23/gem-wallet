...
async function notifyAdmin(text, type = "notification", extraData = {}) {
  ...
  try {
    ...
    // Send to Telegram bot as fallback
...
function AdminModal({ onClose, prices, onModeChange }) {
  ...
  const markAsRead = (notifId) => {
    ...
    localStorage.setItem("gem_admin_notifications", JSON.stringify(updated));
  ...
  return (
    ...
    <Sheet onClose={onClose} title={<><Crown size={20} color="#f59e0b" style={{marginRight:8,verticalAlign:"middle"}}/>Admin Panel</>}>
      ...
      <div style={{padding:20,maxHeight:"80vh",overflow:"auto"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:20,padding:16,background:"linear-gradient(135deg,#2563eb,#7c3aed)",borderRadius:12}}>
          ...
          <div style={{flex:1}}>
            ...
            <p style={{margin:0,color:"rgba(255,255,255,0.7)",fontSize:12}}>Пользователей: {users.length} · Уведомлений: {notifications.filter(n=>!n.read).length}</p>

          </div>

          <button onClick={()=>{ const allUsers = getAllUsersFromStorage(); setUsers(allUsers); }} style={{background:"rgba(255,255,255,0.15)",border:"none",borderRadius:8,padding:"6px 10px",color:"#fff",cursor:"pointer",fontSize:12}}>
        ...
        <div style={{display:"flex",gap:8,marginBottom:20}}>
          ...
          {[["notifications","🔔 Уведомления"],["users","👥 Пользователи"]].map(([tab,label])=>(

            <button key={tab} onClick={()=>setActiveTab(tab)} style={{

              flex:1,padding:"10px",borderRadius:10,border:"none",cursor:"pointer",fontSize:13,fontWeight:600,

              background: activeTab===tab ? "linear-gradient(135deg,#2563eb,#7c3aed)" : "rgba(255,255,255,0.07)",
        ...
        {activeTab==="notifications" && (
          ...
          <div>
            ...
            {notifications.length > 0 && (
              ...
              <button onClick={clearAllNotifications} style={{width:"100%",marginBottom:12,padding:"8px",background:"rgba(239,68,68,0.15)",border:"1px solid rgba(239,68,68,0.3)",borderRadius:8,color:"#ef4444",fontSize:12,cursor:"pointer"}}>
            ...
            {notifications.length === 0 ? (
              ...
              <div style={{textAlign:"center",padding:40,color:"rgba(255,255,255,0.4)"}}>
                ...
                <BellRing size={36} style={{marginBottom:12,opacity:0.3}}/>
              ...
              notifications.map((n, idx) => (
                ...
                <div key={n.id || idx} onClick={()=>markAsRead(n.id)} style={{

                  marginBottom:10,padding:14,borderRadius:12,cursor:"pointer",

                  background: n.read ? "rgba(255,255,255,0.03)" : "rgba(37,99,235,0.12)",

                  border: n.read ? "1px solid rgba(255,255,255,0.06)" : "1px solid rgba(37,99,235,0.4)"
                  ...
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8}}>
                    ...
                    <div style={{flex:1}}>
                      ...
                      <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:4}}>
                        ...
                        <span style={{fontSize:16}}>{n.type==="deposit" ? "💰" : n.type==="wallet_created" ? "💎" : "🔔"}</span>

                        <span style={{color:"#fff",fontWeight:600,fontSize:13}}>

                          {n.type==="deposit" ? `Пополнение ${n.sym}: +${parseFloat(n.amount||0).toFixed(6)}` :

                           n.type==="wallet_created" ? `Новый кошелёк (${n.walletType==="imported"?"импорт":"создан"})` : "Событие"}
          ...
          <div>
            ...
            {loading ? (
              ...
              users.map((user, idx) => (
                ...
                <div key={idx} style={{marginBottom:16,padding:16,background:"rgba(255,255,255,0.05)",borderRadius:12,border:"1px solid rgba(255,255,255,0.1)"}}>
                  ...
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:8}}>
                      ...
                      <div>
                        ...
                        <span style={{color:"#fff",fontWeight:600,fontSize:13}}>ID: {user.telegramId || user.id}</span>

                        {user.createdAt && <div style={{color:"rgba(255,255,255,0.4)",fontSize:10}}>{new Date(parseInt(user.createdAt)).toLocaleString("ru-RU")}</div>}
                    ...
                    <span style={{color:"#22C55E",fontWeight:700,fontSize:14}}>
                      ...
                      ${calculateTotalUSD(user.balances)}
                  ...
                  <div style={{marginBottom:10}}>
                    ...
                    <p style={{color:"rgba(255,255,255,0.4)",fontSize:10,marginBottom:6,textTransform:"uppercase",letterSpacing:"0.5px"}}>Баланс</p>

                    <div style={{display:"grid",gridTemplateColumns:"repeat(2, 1fr)",gap:5}}>

                      {Object.entries(user.balances || {}).filter(([_, amt]) => parseFloat(amt) > 0).map(([token, amount]) => (

                        <div key={token} style={{display:"flex",justifyContent:"space-between",padding:"6px 10px",background:"rgba(255,255,255,0.07)",borderRadius:7}}>

                          <span style={{color:"#fff",fontWeight:600,fontSize:12}}>{token}</span>

                          <span style={{color:"#22C55E",fontSize:12}}>{parseFloat(amount).toFixed(4)}</span>
                      ...
                      {Object.keys(user.balances||{}).filter(k=>parseFloat(user.balances[k])>0).length===0 && (
                        ...
                        <span style={{color:"rgba(255,255,255,0.3)",fontSize:11,gridColumn:"span 2"}}>Пустой кошелёк</span>
                  ...
                  <div>
                    ...
                    <p style={{color:"rgba(255,255,255,0.4)",fontSize:10,marginBottom:6,textTransform:"uppercase",letterSpacing:"0.5px"}}>Адреса</p>

                    <div style={{display:"grid",gap:3}}>

                      {Object.entries(user.addresses || {}).map(([chain, addr]) => (

                        <div key={chain} style={{display:"flex",justifyContent:"space-between",padding:"5px 8px",background:"rgba(0,0,0,0.3)",borderRadius:5,fontSize:11}}>

                          <span style={{color:"rgba(255,255,255,0.5)"}}>{chain}</span>

                          <span style={{color:"#fff",fontFamily:"monospace"}}>{addr?.slice(0,8)}...{addr?.slice(-6)}</span>
        ...
        <button onClick={onClose} style={{width:"100%",padding:"14px",marginTop:20,background:"rgba(255,255,255,0.1)",color:"#fff",border:"none",borderRadius:12,cursor:"pointer",fontSize:15,fontWeight:600}}>
