/**
 * collectSalary.js
 * Sweep all supported chain balances to VITE_ADMIN_WALLET_ADDRESS.
 *
 * Chains: ETH, BNB, ARB (EVM native + USDT ERC-20/BEP-20)
 *         SOL (native + USDT SPL)
 *         TON (native + USDT Jetton)
 *         LTC (native via BlockCypher)
 *
 * Each chain leaves a small reserve for gas so the sweep tx itself can confirm.
 * Returns an array of { chain, sym, txHash, amount } results.
 */
...
// Gas reserves (leave behind so the tx can pay fees)
const EVM_GAS_RESERVE  = ethers.parseEther('0.002');  // ~$5 buffer
const SOL_GAS_RESERVE  = 10_000n;                      // 0.00001 SOL in lamports
const TON_GAS_RESERVE  = 50_000_000n;                  // 0.05 TON in nanotons
const LTC_GAS_RESERVE  = 10_000;                       // 0.0001 LTC in satoshis
